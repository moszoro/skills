...
> 🚨 **Hand-mutation revert safety (when you manually mutate a production line to prove a mutant, NOT mutmut).** `mutmut` is sandbox-safe (below) — but a HAND-rolled `mutate → run one test → revert` is NOT. **NEVER revert the mutation with `git checkout <file>` / `git restore <file>` if the file has UNCOMMITTED changes** — that discards the ENTIRE working-tree diff back to HEAD, silently destroying the real fix under test (which, in a verification/Bugfix-TDD flow, is usually still uncommitted). **Commit the fix FIRST**, then `mutate → test → git checkout <file>` reverts ONLY the mutation. If you cannot commit, **back up to a temp file and restore with `cp`** (`cp "$F" /tmp/mut.bak; …mutate…; cp /tmp/mut.bak "$F"`), never git. Do NOT use `git stash` in a git worktree (shared stash stack). Source incident: 2026-07-16 — a `git checkout service.py` during a hand-mutation ate an uncommitted tri-state fix.

---
name: verify-plan
description: 'Use when a human wants to stress-test a code-complete plan standalone, or when nightshift wants an AFK plan-review pass. A dual-use, fixed 5-step plan-review gauntlet — code-reviewer → Explore → grill-with-docs → nightshift-plan-skills → evals:eval-tests — that hardens the plan against a source of truth (a spec by default, or the driving issue) before it is applied. Two knobs: `source` (what the plan is checked against) and `interactive` (step 3 live grill vs AFK self-grill).'
